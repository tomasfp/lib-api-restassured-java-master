...
public class JDBCOperations {
    ...
    private void queryLog(String query) {
        UtilApi.logger(this.getClass()).log(Level.INFO, "Query >>> {0}", query);
    }
    ...
    public boolean insertNewRow(String schemaTable, String insertColumns, String insertValues) {
        ...
        if (rows > 0) {
            UtilApi.logger(this.getClass()).info("Una nueva fila fue insertada.");
            inserted = true;
        } else {
            UtilApi.logger(this.getClass()).info("Ocurrió un error al insertar una nueva fila.");
        }
        return inserted;
    }
    ...
    public boolean updateValue(String schemaTable, String valuesToUpdate, String condition) {
        ...
        int rows = jdbcTemplate.update(query);
        if (rows > 0) {
            UtilApi.logger(this.getClass()).info("Una nueva fila fue actualizada.");
            updated = true;
        } else {
            UtilApi.logger(this.getClass()).info("Ocurrió un error al actualizar una fila.");
        }
        return updated;
    }
    ...
    public boolean deleteData(String schemaTable, String condition) {
        ...
        int rows = jdbcTemplate.update(query);
        if (rows > 0) {
            UtilApi.logger(this.getClass()).info("Una nueva fila fue eliminada.");
            deleted = true;
        } else {
            UtilApi.logger(this.getClass()).info("Ocurrió un error al eliminar una fila.");
        }
        return deleted;
    }
